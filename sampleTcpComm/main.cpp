...
const option::Descriptor usage[] = {
    { UNKNOWN_OPT, 0,"", "",                Arg::None,
        "Usage: HelloWorldExampleTCP <server|client>\n\nGeneral options:" },
...
int main(int argc, char** argv)
{
    ...
    switch (type)
    {
        case 1:
            {
                TcpServerPubSub tcpServer;
                if (tcpServer.init(wan_ip, static_cast<uint16_t>(port), use_tls, whitelist))
                {
                    tcpServer.run(count, sleep);
                }
                break;
            }
        case 2:
            {
                TcpClientPubSub tcpClient;
                if (tcpClient.init(wan_ip, static_cast<uint16_t>(port), use_tls, whitelist))
                {
                    tcpClient.run();
                }
                break;
